...
type FormatOptions struct {
	...
	HideWrapFrames bool // 启用隐藏 wrap 帧
}
...
func Unpack(err error) UnpackedError {
	...
	for err != nil {
		...
		case *wrapError:
			...
			allLinks = append([]ErrLink{link}, allLinks...) // prepend
		default:
	...
	for _, link := range allLinks {
		if link.Msg != "" { // 只保留有消息的 wrap 帧
			upErr.ErrChain = append(upErr.ErrChain, link)
		}
	}
...
// 新增方法：判断是否为空的 wrap 帧
func (eLink *ErrLink) IsEmptyWrap() bool {
	return eLink.Msg == ""
}
